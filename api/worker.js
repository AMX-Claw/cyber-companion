  ...
  async fetch(request, env) {
    ...
    try {
      ...
      if (path === '/diary' && request.method === 'GET') {
        ...
        // 同时获取探险日记
        const adventures = await db.prepare(
          "SELECT completed_at AS date, diary_text AS content, loot_name FROM adventures WHERE baby_id = ? AND status = 'completed' AND diary_text IS NOT NULL ORDER BY completed_at DESC LIMIT 30"
        ).bind(babyId).all();


        // 合并：探险日记标记为探险类型，转换timestamp为日期字符串
        const adventureDiaries = (adventures.results || []).map(a => {
          const d = new Date(Number(a.date));
          const dateStr = d.toISOString().split('T')[0];
          return {
            date: dateStr,
            content: `🗺️ 探险日记：${a.content}`,
            day: null,
            type: 'adventure'
          };
        });

        const allDiaries = [...(diaries.results || []).map(d => ({ ...d, type: 'daily' })), ...adventureDiaries];
        // 按日期倒序排
        allDiaries.sort((a, b) => b.date.localeCompare(a.date));

        return json({ diaries: allDiaries.slice(0, 30) });
      }
...
只输出这句话本身，不要引号不要其他任何文字。`;
      ...
      if (path === '/events/available' && request.method === 'GET') {
        ...
        // 优先触发当天的固定剧情（生日趴等不该被卡住）
        // 否则按顺序找最早的未完成剧情
        const eligibleDays = storyDays.filter(d => d <= day);
        let availableStory = null;
        if (storyIdMap[day] && !completedIds.includes(storyIdMap[day])) {
          availableStory = day;
        } else {
