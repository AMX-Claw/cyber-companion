...
function CoralBabyGame() {
  ...
  const ambientFetchedAtRef = useRef(0);

  // Gemini-生成的当前状态台词（替代硬编码默认话）。5 分钟或关键状态变化时刷新。
  useEffect(() => {
    if (!gameState) return;
    const now = Date.now();
    if (now - ambientFetchedAtRef.current < 5 * 60 * 1000) return;
    ambientFetchedAtRef.current = now;
    const token = localStorage.getItem('xiaoke_auth_token');
    if (!token) return;
    const base = window.APP_CONFIG?.API_BASE || 'https://xiaoke-api.megyucai.workers.dev';
    fetch(`${base}/ambient-quote`, { headers: { 'Authorization': `Bearer ${token}` } })
      .then(r => r.ok ? r.json() : null)
      .then(data => { if (data?.quote) setAmbientQuote(data.quote); })
      .catch(() => {});
  }, [gameState?.day, Math.floor((gameState?.happiness || 0) / 20), Math.floor((gameState?.cleanliness || 0) / 20), Math.floor((gameState?.hunger || 0) / 20)]);

  const getDailyMsg = (s) => {
    if (ambientQuote) return ambientQuote;
  ...
  const completeStoryEvent = (eid,resp) => {
    if(!gameState) return;
    const actualDay = calcDay(gameState.createdAt, gameState.frozenDays||0);
    const mems=resp.filter(r=>r.type==='user'||r.type==='baby').map(r=>({type:r.type,text:r.text}));
    const nm={eventId:eid,day:actualDay,title:showStoryEvent?.title||eid,memories:mems,date:new Date().toISOString()};
    const ns={...gameState,day:actualDay,completedEvents:[...(gameState.completedEvents||[]),eid],storyMemories:[...(gameState.storyMemories||[]),nm]};
    setGameState(ns); saveGame(ns, true); setStoryComplete(true);
    // Save memory to API
    if (!offlineMode && window.XiaokeAPI?.getAuthToken()) {
      try { window.XiaokeAPI.saveMemory(eid, actualDay, showStoryEvent?.title || eid, false, mems); } catch(e) {}
    }
  };
  ...
  const skipStory = () => { if(!showStoryEvent) return; const ad=calcDay(gameState?.createdAt,gameState?.frozenDays||0); const m=storyResponses.filter(r=>r.type==='user'||r.type==='baby').map(r=>({type:r.type,text:r.text})); if(m.length>0&&gameState){const nm={eventId:showStoryEvent.id,day:ad,title:showStoryEvent.title,memories:m,date:new Date().toISOString()};const ns={...gameState,day:ad,completedEvents:[...(gameState.completedEvents||[]),showStoryEvent.id],storyMemories:[...(gameState.storyMemories||[]),nm]};setGameState(ns);saveGame(ns);} setShowStoryEvent(null);setCurrentEventDay(null);setStoryStep(0);setStoryResponses([]);setStoryComplete(false); };
  ...
  const closeRandom = () => {
    if(gameState&&randomEventResponses.length>0) {
      ...
      const nm={eventId:showRandomEvent.id,day:actualDay,title:showRandomEvent.title,memories:m,date:new Date().toISOString(),isRandom:true};
      const ns={...gameState,day:actualDay,storyMemories:[...(gameState.storyMemories||[]),nm]};
      if(!showRandomEvent.hasPhoto||!photoUploaded) ns.coins=(ns.coins||0)+(showRandomEvent.coins||15);
      setGameState(ns); saveGame(ns, true);
      // Save memory to API
      if (!offlineMode && window.XiaokeAPI?.getAuthToken()) {
        try { window.XiaokeAPI.saveMemory(showRandomEvent.id, actualDay, showRandomEvent.title, true, m); } catch(e) {}
      }
    }
